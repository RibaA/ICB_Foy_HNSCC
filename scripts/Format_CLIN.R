...
cols <- c('geo_accession', 'Sex.ch1', 'age.ch1', 'site.ch1', 'best.response.on.immunotherapy..recist..ch1',
          'os..event..ch1', 'os..month..ch1', 'pfs..event..ch1', 'pfs..month..ch1')

clin <- clin[, c(cols, colnames(clin)[!colnames(clin) %in% cols])]
colnames(clin)[colnames(clin) %in% cols] <- c('patient', 'sex', 'age', 'histo', "recist",
                                              'os', 't.os', "pfs", "t.pfs")

new_cols <- c( "primary" , "stage" , "response.other.info" , "response" , "drug_type" , "dna" , "rna", "rna.info")
clin[new_cols] <- NA
...
clin <- annotate_tissue(clin=clin, study='Foy_HNC', annotation_tissue=annotation_tissue, check_histo=FALSE)
